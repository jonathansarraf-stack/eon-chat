...
async function listChatSessions(db, tenantId, workspaceId, userId) {
...
async function createChatSession(db, { tenantId, workspaceId, userId, providerAccountId, title, metadata }) {
  const result = await db.query(
    `insert into chat_sessions (tenant_id, workspace_id, user_id, provider_account_id, title, metadata_json)
     values ($1, $2, $3, $4, $5, $6::jsonb)
     returning id, tenant_id, workspace_id, user_id, provider_account_id, title, status, metadata_json, created_at, updated_at`,
    [tenantId, workspaceId, userId, providerAccountId || null, title || 'New chat', JSON.stringify(metadata || {})]
  );
...
async function getChatSession(db, sessionId) {
  const result = await db.query(
    `select id, tenant_id, workspace_id, user_id, provider_account_id, title, status, metadata_json, created_at, updated_at
...
async function listRuns(db, sessionId) {
  const result = await db.query(
    `select id, tenant_id, workspace_id, session_id, provider_account_id, provider, model, status,
...
async function createRun(db, { tenantId, workspaceId, sessionId, providerAccountId, provider, model, executionStats }) {
  const result = await db.query(
    `insert into runs (tenant_id, workspace_id, session_id, provider_account_id, provider, model, status, execution_stats_json)
     values ($1, $2, $3, $4, $5, $6, 'queued', $7::jsonb)
     returning id, tenant_id, workspace_id, session_id, provider_account_id, provider, model, status,
               sandbox_id, started_at, finished_at, cost_usd, token_usage_json, execution_stats_json, created_at, updated_at`,
    [tenantId, workspaceId, sessionId, providerAccountId || null, provider, model, JSON.stringify(executionStats || {})]
  );
  return result.rows[0];
}
  ...
  const result = await db.query(
    `with next_run as (
     ...
     update runs r
      ...
      returning r.id, r.tenant_id, r.workspace_id, r.session_id, r.provider_account_id, r.provider, r.model,
...
async function listRecentProblemRuns(db, { tenantId, limit = 5 } = {}) {
  ...
  const result = await db.query(
    `select id, tenant_id, workspace_id, session_id, provider, model, status, execution_stats_json, updated_at
     from runs
