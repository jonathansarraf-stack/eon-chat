...
const { randomToken, hashToken, hashPassword, verifyPassword, needsPasswordRehash } = require('../crypto');
...
async function signIn({ email, password, ipAddress, userAgent }) {
  ...
  const replacementPasswordHash = needsPasswordRehash(user.password_hash)
  ...
  const result = await withTransaction(async (db) => {
    ...
    if (replacementPasswordHash) {
      await usersRepo.updatePasswordHash(db, user.id, replacementPasswordHash);
    }
    const rawToken = randomToken();
    const hashedToken = hashToken(rawToken);
    const expiresAt = sessionExpiryDate();
